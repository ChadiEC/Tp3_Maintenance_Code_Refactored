...
public class McDonaldSystem {
    ...
    public static ArrayList<CartItemOld> cart = new ArrayList<>(); // Le panier global
            ...
            if (choice == 1) {
                ...
                if (m >= 0 && m < mains.size() && s >= 0 && s < snacks.size() && d >= 0 && d < drinks.size()) {
                    ...
                    if (mains.get(m).stock > 0 && snacks.get(s).stock > 0 && drinks.get(d).stock > 0) {
                        CartItemOld trio = new CartItemOld(mains.get(m), snacks.get(s), drinks.get(d));
                        cart.add(trio);
                ...
                if (itemChoice >= 0 && itemChoice < inventory.size()) {
                    ...
                    if (selectedItem.stock > 0) {
                        CartItemOld cartItem = new CartItemOld(selectedItem);
                        cart.add(cartItem);
                ...
                if (cart.size() == 0) {
                    ...
                    if (removeChoice > 0 && removeChoice <= cart.size()) {
                        CartItemOld removed = cart.remove(removeChoice - 1);
